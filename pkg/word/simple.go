...
// SimpleDocx support manipulate docx files with all built-in golang library
type SimpleDocx struct {
	t *DocxReplacer
}

func (r *SimpleDocx) OpenTemplate(path string) (DocxTemplate, error) {
	// Validation
	if !strings.HasSuffix(path, ".docx") {
		return nil, errors.New("incorrect file format")
	}
	// Open template
	template, err := ReadDocxFile(path)
	if err != nil {
		return nil, err
	}
	return &SimpleDocx{template}, nil
}

func (r *SimpleDocx) RenderTemplate(data interface{}) error {
	// Read all struct tags
	// TODO: [Anh Le] Read all struct tags
	// Replace all placeholders by data mapping with tags
	// TODO: [Anh Le] Replace all placeholders by data mapping with tags
	return nil
}

func (r *SimpleDocx) Save(filePath string) error {
	//r.t.WriteToFile("./new_result_1.docx")
	return nil
}

func (r *SimpleDocx) ResetTemplate() error {
	return nil
}

func (r *SimpleDocx) CloseTemplate() error {
	return nil
}
...
func (r *DocxReplacer) Editable() *Docx {
	return &Docx{
...
func (r *DocxReplacer) Close() error {
...
func ReadDocxFromFS(file string, fs fs.FS) (*DocxReplacer, error) {
	f, err := fs.Open(file)
	if err != nil {
		return nil, err
	}
...
func ReadDocxFromMemory(data io.ReaderAt, size int64) (*DocxReplacer, error) {
...
func ReadDocxFile(path string) (*DocxReplacer, error) {
...
func ReadDocx(reader ZipData) (*DocxReplacer, error) {
	...
	return &DocxReplacer{zipReader: reader, content: content, links: links, headers: headers, footers: footers, images: images}, nil
}
...
//	To get Word to recognize a tab character, we have to first close off the previous text element.
//
// This means if there are multiple consecutive tabs, there are empty <w:t></w:t>
